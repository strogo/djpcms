...
class S3BotoStorage(Storage):
    ...
    def content_type(self, content):
        ...
        #ctype = mimetypes.guess_type(content.file)[0]
        #if not ctype:
        #    ctype = 'text/plain'
        #return ctype
    ...
    def _save(self, name, content):
        ...
        headers['Content-Type'] = self.content_type(content)
        k = self.bucket.get_key(name)
        if not k:
            k = self.bucket.new_key(name)
        k.set_contents_from_file(content.file, headers=headers, policy=self.acl)
        return name
